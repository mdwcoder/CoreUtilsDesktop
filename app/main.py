...
WEB_SECTIONS: list[tuple[str, str, str, str]] = [
    ...
    ("forge", "/forge", "web_forge", ft.Icons.HANDYMAN_ROUNDED),
    ("connect", "/connect", "web_connect", ft.Icons.FORUM_ROUNDED),
    ("relay", "/relay", "web_relay", ft.Icons.SEND_ROUNDED),
    ("easyforms", "/easyforms", "web_easyforms", ft.Icons.ARTICLE_ROUNDED),
    ("devs", "/devs", "web_devs", ft.Icons.GROUP_ROUNDED),
]
...
TRANSLATIONS = {
    "es": {
        ...
        "web_connect": "Connect",
        "web_relay": "Relay",
        "web_easyforms": "EasyForms",
        "web_devs": "Devs",
        "web_open_tooltip": "Abrir {name} en una ventana",
        "web_open_start": "Abriendo {name}...",
        "web_open_fail": "No se pudo abrir {name}: {error}",
    },
    "en": {
        ...
        "web_easyforms": "EasyForms",
        "web_devs": "Devs",
        "web_open_tooltip": "Open {name} in a window",
        "web_open_start": "Opening {name}...",
        "web_open_fail": "Could not open {name}: {error}",
    },
}
...
class CoreUtilsDesktop:
    ...
    def _web_apps_group(self) -> ft.Control:
        return ft.Container(
            padding=theme.pad(horizontal=6, vertical=4),
            border_radius=12,
            bgcolor=theme.SURFACE,
            ...
            content=ft.Row(
                ...
                controls=[
                    theme.ghost_button(
                        icon,
                        self._t("web_open_tooltip", name=self._t(label_key)),
                        lambda _e, slug=slug, path=path, label_key=label_key: self._open_web_section(slug, path, label_key),
                    )
                    for slug, path, label_key, icon in WEB_SECTIONS
                ],
            ),
        )
    ...
    def _open_web_section(self, _slug: str, path: str, label_key: str) -> None:
        ...
        try:
            ...
            self._append_log(self._t("web_open_fail", name=name, error=str(exc)), accent=theme.RED)
